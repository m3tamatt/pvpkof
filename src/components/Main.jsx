...
export default class Main extends React.Component {
  ...
  getStream() {
    ...
    if (typeof navigator.mediaDevices.getUserMedia === 'undefined') {
      // Disable audio and video constraints
      navigator.getUserMedia({}, gotMedia, () => this.setState({ localVideo: false }));
    } else {
      // Disable audio and video constraints
      navigator.mediaDevices.getUserMedia({}).then(gotMedia).catch(() => this.setState({ localVideo: false }));
    }
  }
  ...
  <div style={{ 
    ...
    {this.state.localVideo ? (
      ...
      <img src="https://s11.gifyu.com/images/PVP-Zone.gif" style={{ maxWidth: '100%', maxHeight: '100%' }} />
    )}
  </div>
</div>
        </div>
